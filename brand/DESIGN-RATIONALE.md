...
## In-app color system — Mono by default, Flame by choice

Decided 2026-08-11 (research: HIG icon/tint coordination; Stripe's
shift-the-shade accessibility method; WCAG 2.1 contrast). Shipped first in
...
Every app ships two themes behind a Settings → Appearance "App Color"
segmented picker (`@AppStorage "appColorTheme"`; user-facing labels
**Classic** / **Flame**). The home-screen icon is never affected.

- **Classic (default)** — a warm ink-and-paper scheme in the tone of the
  Claude app: neutral chrome; primary buttons are ink `#211D19` with paper
  `#F7F5F1` labels (inverted in dark); the app's flame hue appears only
  where it carries meaning — links, tab selection, toggles, selection
  marks, one key data highlight per screen. Color regains meaning by being
  scarce, and Classic still whispers which app you're in.
- **Flame (opt-in)** — the accent turned up everywhere, starting with vivid
  flame-hue primary CTAs.
...
app so the siblings stay identical.
